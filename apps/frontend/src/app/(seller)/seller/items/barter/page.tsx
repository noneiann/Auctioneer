...
import { X, Plus, ImageIcon, ArrowLeftRight, ChevronLeft, DollarSign } from "lucide-react";
...
export default function BarterItemPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const resp = await api.post("/items", {
				...
				type: "BARTER",
				price: parseFloat(formData.estimatedValue),
				imageUrl: images,
			});
			if (resp && resp.success) {
				router.push("/seller/items");
			} else {
				throw new Error(resp?.data || "Failed to create barter listing");
			}
		} catch (err) {
			setError(err instanceof Error ? err.message : "Failed to create barter listing");
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<div className="max-w-2xl mx-auto">
			...
			<div className="mb-8">
				...
				<p className="text-[#737373] text-sm mt-1">Offer an item for exchange. Others will propose trades.</p>
			</div>
			...
			<form onSubmit={handleSubmit} className="space-y-5">
				...
				<section className="bg-[#181818] border border-[#1f1f1f] rounded-xl p-6 space-y-5">
					...
					<div>
						...
						<input type="text" name="name" value={formData.name} onChange={handleChange}
							required placeholder="What are you offering for trade?"
					...
					<div>
						...
						<textarea name="description" value={formData.description} onChange={handleChange}
							required rows={4} placeholder="Describe condition, age, any flaws..."
					...
					<div>
						<label className={labelClass}>What are you looking for? <span className="normal-case font-normal text-[#4a4a4a]">(optional)</span></label>
						<input type="text" name="lookingFor" value={formData.lookingFor} onChange={handleChange}
							placeholder="e.g. Mechanical keyboard, vintage camera, sneakers..."
							className={inputClass} />
					</div>
				</section>
					...
					{images.length === 0 ? (
						<div
							...
							className={`flex flex-col items-center justify-center gap-3 border-2 border-dashed rounded-xl py-10 cursor-pointer transition-colors ${
								dragOver ? "border-purple-500 bg-purple-500/5" : "border-[#2a2a2a] hover:border-[#3a3a3a]"
							}`}
						>
				...
				{/* Valuation */}
				<section className="bg-[#181818] border border-[#1f1f1f] rounded-xl p-6 space-y-5">
					<p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#4a4a4a]">Valuation</p>

					<div>
						<label className={labelClass}>Estimated Value</label>
						<div className="relative">
							<DollarSign className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4a4a4a]" />
							<input type="number" name="estimatedValue" value={formData.estimatedValue}
								onChange={handleChange} required min="1" step="0.01" placeholder="0.00"
								className={`${inputClass} pl-9 text-lg font-mono`} />
						</div>
						<p className="text-[11px] text-[#4a4a4a] mt-2">
							Helps others propose fair, equivalent trades. Not a selling price.
						</p>
					</div>
				</section>
				...
				<button type="submit" disabled={loading}
					...
					{loading ? "Publishing…" : "Publish Trade Offer"}
				</button>
			</form>
		</div>
	);
